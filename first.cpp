...
int main( int argc, char *argv[ ] )
{
	...
	cl_uint devices;
	status = clGetDeviceIDs(platform[1], CL_DEVICE_TYPE_GPU, MAX_DEVICES, device, &devices);
	if( status != CL_SUCCESS )
	...
	cl_context context = clCreateContext( NULL, 1, &device[0], NULL, NULL, &status );
	if( status != CL_SUCCESS )
	...
	status = clBuildProgram( program, 1, &device[0], options, NULL, NULL );
	if( status != CL_SUCCESS )
	{
		size_t size;
		clGetProgramBuildInfo( program, device[0], CL_PROGRAM_BUILD_LOG, 0, NULL, &size );
		cl_char *log = new cl_char[ size ];
		clGetProgramBuildInfo( program, device[0], CL_PROGRAM_BUILD_LOG, size, log, NULL );
