...
kernel void ArrayMultAdd(global const float *dA, global const float *dB, global const float *dC, global float *dD)
{
	int gid = get_global_id(0);
...
kernel void ArrayMultReduceDouble(global const double *dA, global const double *dB, local double *prods, global double *dC)
{
	int gid = get_global_id(0);
	int numItems = get_local_size(0);
	int tnum = get_local_id(0);
